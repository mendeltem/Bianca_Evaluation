...
# -*- coding: utf-8 -*-
"""
Metrics by Test Condition + Bland-Altman (B+L only)
======================================================

Revision context (NeuroImage: Clinical, Major Revision)
-------------------------------------------------------
Violin plots (one per metric: Dice Score, Sensitivity, Precision)
comparing performance across three test conditions (non_removed,
removed, inpainted) with training condition fixed to inpainted.
...
B+L threshold, comparing all three test condition pairs:
  1. Non Removed vs Removed
  2. Non Removed vs Inpainted
  3. Removed vs Inpainted

Addresses: R1 Comment 1, R5 #9 (preprocessing effect on test images).

Design: Models trained on inpainted condition (fixed), threshold B+L, n=89 subjects.
Per-subject means averaged across 10 seeds x 5-fold stratified CV.

Key results
-----------
Test condition preprocessing produced statistically detectable but
negligible differences in Sensitivity and Precision, while Dice Score
remained unaffected. All Cliff's delta were negligible (max |delta| =
0.0030), confirming that the detected differences have no practical
significance despite reaching statistical significance due to the high
power of the paired design (89 subjects x 10 seeds x 5-fold CV).

Violin plots (B+L threshold):
  Dice Score:  Mean = 0.567 +/- 0.236 (identical across all three
               conditions to 3rd decimal). All pairwise p_bonf >= 0.767.
  Sensitivity: Mean = 0.687 +/- 0.179. All three pairwise comparisons
               significant after Bonferroni (NR vs R: p<0.001***,
               NR vs Inpainted: p=0.004**, R vs Inpainted: p=0.001**).
               Despite significance, all Cliff's delta negligible
               (max |delta| = 0.0030).
  Precision:   Mean = 0.557 +/- 0.287. NR vs R: p=0.001**, NR vs
               Inpainted: p=0.007**. R vs Inpainted: p=0.080 (ns).
               All Cliff's delta negligible (max |delta| = 0.0024).

  Interpretation: The high statistical power of the averaged 10-seed
  5-fold design detects sub-0.001-point differences in Sensitivity and
  Precision. These are statistically significant but negligible in
  magnitude. Dice Score, which integrates both Sensitivity and Precision,
  shows no significant differences, indicating that the opposing
  directional shifts in Sensitivity (Removed > Non Removed) and
  Precision (Non Removed > Removed) cancel out.
...
Bland-Altman agreement (B+L threshold):
  95% LoA were extremely narrow, even narrower than the training
  condition comparison:
    Non Removed vs Removed:   Dice [0, 0], Sens [-0.001, 0.001],
                              Prec [-0.001, 0.001]
    Non Removed vs Inpainted: Dice [0, 0], Sens [-0.001, 0],
                              Prec [-0.001, 0.001]
    Removed vs Inpainted:     Dice [0, 0], Sens [0, 0.001],
                              Prec [-0.002, 0.001]

  Maximum bias across all 9 panels: 0.000204 (Precision NR vs R).
  Maximum LoA width: 0.003 (Precision R vs Inpainted).

  6 of 9 uncorrected bias tests reached p < 0.05, consistent with the
  Wilcoxon violin results. All Cliff's delta remained negligible
  (max |delta| = 0.0030). The systematic bias pattern reflects the
  mechanistic effect of lesion presence on intensity normalization:
  removing the stroke lesion slightly increases Sensitivity (BIANCA
  detects previously obscured WMH) while slightly decreasing Precision
  (more false positives from normalization changes).

Paper changes
-------------
  - Section 3.2 (Results): Violin + BA figures for test condition
    comparison. Key finding: test condition preprocessing produces
    statistically detectable but negligible effects (all |delta| < 0.003).
...
---------------------
  R1 Comment 1: "Test condition preprocessing (non_removed, removed,
    inpainted) produced statistically detectable differences in
    Sensitivity and Precision (Bonferroni-corrected p < 0.01 for 5/9
    comparisons), but all Cliff's delta were negligible (max |delta| =
    0.003). Dice Score showed no significant differences (all p_bonf
    >= 0.767). Bland-Altman 95% LoA were within +/-0.002 across all
    metrics, confirming that test condition effects are negligible in
    magnitude despite statistical detectability."

  R5 Comment 9: "The opposing directional pattern in Sensitivity
    (Removed > Non Removed) and Precision (Non Removed > Removed)
    is consistent with the mechanistic explanation: removing the stroke
    lesion allows BIANCA's kNN classifier to detect previously obscured
    subtle WMH (higher Sensitivity), but the altered intensity
    normalization also introduces additional false positives (lower
    Precision). These effects cancel in Dice Score and are negligible
    in magnitude (all |delta| < 0.003)."

  R5 Comment 4: "Bland-Altman plots confirm that the maximum difference
    between any two test conditions is 0.0002 Dice points, with 95% LoA
    of +/-0.001 or narrower for all Dice comparisons. These differences
    fall well below the measurement precision of WMH segmentation."

Outputs
-------
  dice_score_test_conditions_locate.png
  sensitivity_test_conditions_locate.png
  precision_test_conditions_locate.png
  BA_non_removed_vs_removed_test_locate.png
  BA_non_removed_vs_filled_test_locate.png
  BA_removed_vs_filled_test_locate.png
  locate_test_conditions_results.xlsx
...
output_dir = os.path.join(SCRIPT_DIR, "analysis", "dice_locate_test_conditions")
os.makedirs(output_dir, exist_ok=True)

# Fixed design
TRAIN_CONDITION = 'filled'   # inpainted training (fixed)
THRESHOLD = 'locate'

# Three test conditions (x-axis)
...
def load_and_aggregate(df):
    """
    Filter pre-loaded DataFrame to inpainted training + B+L threshold,
    aggregate per subject x test_condition.
    """
    df = df[(df['train_condition'] == TRAIN_CONDITION) &
            (df['threshold'] == THRESHOLD)].copy()

    print(f"Filtered: {len(df)} rows | Test conditions: "
          f"{sorted(df['test_condition'].unique())}")

    group_cols = ['subject', 'test_condition']
...
def compute_shared_violin_ylim(df_agg):
    ...
    for metric in VIOLIN_METRICS:
        for cond in COND_ORDER:
            vals = df_agg[df_agg['test_condition'] == cond][metric].dropna().values
            if len(vals) > 0:
...
def compute_wilcoxon_pairwise(df_agg):
    ...
    for metric in VIOLIN_METRICS:
        ...
        for c1, c2 in combinations(COND_ORDER, 2):
            ...
            d1 = df_agg[df_agg['test_condition'] == c1][['subject', metric]].set_index('subject')
            d2 = df_agg[df_agg['test_condition'] == c2][['subject', metric]].set_index('subject')
...
def plot_metric_conditions(df_agg, metric, metric_label, ylim):
    """
    Violin plot: given metric across three test conditions,
    ...
    for i, cond in enumerate(COND_ORDER):
        sub = df_agg[df_agg['test_condition'] == cond]
        pos = positions[i]
    ...
    ax.set_xlabel('Test Condition (Preprocessing)', fontsize=12, labelpad=12)
    ax.set_ylabel(metric_label, fontsize=12)
    ax.set_title(f"Phase I: {metric_label} by Test Condition (B+L, trained on Inpainted)",
    ...
    if sig_brackets:
        all_vals = np.concatenate([
            df_agg[df_agg['test_condition'] == c][metric].dropna().values
            for c in COND_ORDER])
    ...
    fname = f"{metric}_test_conditions_{THRESHOLD}"
...
def plot_ba_comparison(panels, cond1, cond2, ylim):
    ...
    fig.suptitle(f'Phase I: Bland-Altman: {label1}  vs  {label2}  (B+L, trained on Inpainted)',
    ...
    fig.tight_layout()
    fname = f"BA_{cond1}_vs_{cond2}_test_locate"
    fig.savefig(os.path.join(output_dir, f"{fname}.png"), dpi=300, bbox_inches='tight')
    plt.close(fig)
    print(f"  Saved: {fname}.png")
...
print("=" * 70)
print("PHASE I: METRICS BY TEST CONDITION (B+L, TRAINED ON INPAINTED)")
print(f"Train: Inpainted | Threshold: B+L")
print(f"Test conditions: {[COND_LABELS[c] for c in COND_ORDER]}")
...
for metric in VIOLIN_METRICS:
    metric_label = VIOLIN_METRIC_LABELS[metric]
    print(f"\n  Violin: {metric_label}")
    plot_metric_conditions(df_agg, metric, metric_label, violin_ylim)
...
# ── BA analysis ──
print(f"\n{'─' * 50}")
print(f"  BLAND-ALTMAN ANALYSIS (B+L, trained on Inpainted)")
print(f"{'─' * 50}")
...
xlsx_path = os.path.join(output_dir, "locate_test_conditions_results.xlsx")
...
meta = {
    'Wilcoxon Pairwise': [
        "Table: Phase I - Wilcoxon Signed-Rank Pairwise Tests (Test Conditions)",
        f"Design: Models trained on {COND_LABELS[TRAIN_CONDITION]} condition, threshold B+L.",
    ...
    'Descriptive': [
        "Table: Phase I - Descriptive Statistics (Test Conditions)",
        f"Design: Models trained on {COND_LABELS[TRAIN_CONDITION]} condition, threshold B+L.",
        "95% CI via bootstrap (1000 iterations).",
        "Per-subject means across 10 seeds x 5-fold stratified CV.",
    ],
    'Bland-Altman': [
        "Table: Phase I - Bland-Altman Agreement (Test Conditions, B+L Threshold)",
        f"Design: Models trained on {COND_LABELS[TRAIN_CONDITION]} condition, threshold B+L.",
        ...
        "Marginal p-values (0.03-0.05) should be interpreted with caution; check Cliff's delta magnitude.",
...
wilcoxon_img = os.path.join(output_dir, "wilcoxon_test_conditions.jpg")
table_to_image(wilcoxon_df, wilcoxon_img)
desc_img = os.path.join(output_dir, "descriptive_test_conditions.jpg")
table_to_image(descriptive_df, desc_img)
ba_img = os.path.join(output_dir, "bland_altman_test_stats.jpg")
