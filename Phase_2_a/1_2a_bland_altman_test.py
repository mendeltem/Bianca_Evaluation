...
Design: train=inpainted (fixed, single model), threshold=LOCATE.
...
def plot_ba_comparison(panels, cond1, cond2, ylim):
    ...
    if has_lesion_type:
        ...
        legend_handles.append(plt.Line2D([0], [0], color='gray', marker='x',
                                          linestyle='None', markersize=8, label='Outlier'))
...
if __name__ == "__main__":
    ...
    print(f"Train: {COND_LABELS[TRAIN_CONDITION]} (fixed) | Threshold: LOCATE")
    ...
    meta = {
        'Bland-Altman': [
            ...
            f"Design: train={COND_LABELS[TRAIN_CONDITION]}, threshold=LOCATE",
